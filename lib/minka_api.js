...
const MinkaAPI = class MinkaAPI {
  ...
  static post(route, p, opts) {
    ...
    if (options.upload) {
      ...
      console.debug("Multipart params:", params, interpolated.remainingParams);
      Object.keys(params).forEach(k => {
        // FormData params can include options like file upload sizes
        if (params[k] && params[k].type === "custom" && params[k].value) {
          // Extract filename from the stream path if available
          const value = params[k].value;
          const options = params[k].options || {};

          // If it's a ReadStream, extract the filename from the path
          if (value.path) {
            const pathParts = value.path.split('/');
            const filename = pathParts[pathParts.length - 1];
            // Append with filename as the third parameter for proper file upload
            body.append(k, value, { filename, ...options });
          } else {
            body.append(k, value, options);
          }
        } else {
